# MEC Daily Training & Nutrition Log

**Date:** Thursday, 6 August 2026
**Athlete:** Mohamad Eid Chmaitilli (MEC)
**Prepared for:** Cal (personal trainer) / coaching team
**Status:** OPEN. Filled as the day runs.

---

## 1. Athlete Snapshot

| Field | Value |
|---|---|
| Weight | Not reported |
| Last measured | 88.9 kg, empty stomach, 2 August |
| Session | **ARMS scheduled.** EZ curl 30 kg anchor. No shrugs, no upright rows |
| Sleep | Not yet reported. Bedtime range 23:00–23:30 stated yesterday |
| Enhancement status | On cycle — boldenone and testosterone |
| Phase | Diet break, final full day. **Cut begins Saturday 8 August at ~2200 kcal** |
| Headache | Status not yet reported. Yesterday: bilateral, AC-draft-led differential |

---

## 2. GI: THE DAY-THREE READ IS IN, AND IT OVERSHOT

**Reported: almost no movement — very small pieces, heavy pushing, and a persistent sense that the gut is full and needs to move.**

### What this is

**This is not the loose-stool problem continuing. It is the original problem returning.**

| Symptom today | Late-July baseline (27 Jul – 2 Aug) |
|---|---|
| Very small pieces | Hard, fragmented stool |
| Had to push a lot | Straining on nearly every passage |
| Gut feels full, needs movement | Incomplete evacuation, ~15 passages in a row |

**Point for point, today matches the picture the log opened with — the one that predates the bars coming out.**

### The three-day arc, completed

| Day | Bar-free day | Read |
|---|---|---|
| 4 August | 1 | Swooshy AM, two formed pieces PM |
| 5 August | 2 | Half formed, loose at the end |
| **6 August** | **3** | **Small hard pieces, straining, incomplete** |

**The trend did not level off at normal. It went loose → half-formed → hard in three days.**

### What the test actually answered

**The bar test has read out, one day early and more sharply than designed:**

1. **The polyols were driving the looseness.** Consistency swung from swooshy to hard within three days of removal — that speed is the signature of an osmotic agent leaving the system, not of a fibre adaptation.
2. **Underneath, the baseline gut is constipation-prone.** The bars were not just causing loose stool — **they were accidentally treating the hard-stool tendency at the same time**, by pulling water into the colon every day. Remove them and the late-July pattern resurfaces almost immediately.

**Both things were true at once.** The 26–36 g daily polyol load overshot: it turned a constipation-prone gut into a loose one. Removing it swung the pendulum straight back.

**His 2 August comment — "I feel got is full always" — now reads as part of this picture**, present even during the loose phase.

### What changes today

**START PSYLLIUM TODAY, not Friday.** The only reason it was deferred was to keep the bar test clean, and the test has now answered. There is nothing left to confound.

- **Dose: start at 1 teaspoon (~5 g) in a full glass of water, once today**, ideally with a meal. Ramp toward twice daily over the next week if tolerated
- **Water is non-negotiable with it.** Psyllium without fluid makes hard stool worse. Yesterday's ~2.5 L is the number to repeat, and in this heat it matters double
- **Do not strain.** Pushing hard against small hard stool is how haemorrhoids and fissures happen. If it does not come, stand up and try later — the urge returns with the gastrocolic reflex after meals
- **Figs and walnuts stay.** Their fibre is now working with the plan, not against a test
- **Bars stay out.** The answer to hard stool is fibre and water, not reintroducing a 30 g polyol load
- **Magnesium bisglycinate stays for sleep.** It is not the tool for this — but if psyllium plus water has not moved things in 3–4 days, **magnesium citrate at night is the correct next lever**, and that decision goes through Dr Chalhoub

### What does not change

**No alarm.** Yesterday produced one well-relieved passage; today's poor one reflects a single transition day, not a week of failure. The fibre base (~21 g yesterday, from real food) is the best it has been, and psyllium starts on top of it today. **Give the combination 3–4 days before judging it.**

---

## 3. Nutrition Log

### Morning

| Item | P | C | F | kcal |
|---|---|---|---|---|
| Half protein cookie | 5 | 6 | 1.5 | ~50 |
| Mhalabieh, small (~80% milk) | ~3 | ~15 | ~3 | ~100 |
| Coffee, 1 normal + 1 decaf capsule | 0 | 0 | 0 | 0 |
| **Total** | **~8** | **~21** | **~4.5** | **~150** |

**Caffeine: ~60–80 mg. Well under the cap** — one normal capsule plus a decaf is a light load, and with the headache differential still open that is the right side to be on.

**Fourth consecutive light opening (~150 kcal), and today it matters more than yesterday** — this is an Arms day. The midday meal has to be real: yesterday proved protein-at-noon works, and today needs the same structure. **~190 g of protein still to find, and a session to fuel.**

### Meal 2 — post-workout

| Item | P | C | F | kcal |
|---|---|---|---|---|
| Protein shake + banana, blended | ~31 | ~30 | ~2 | ~255 |
| Brookie × 1.5 | 13.5 | 18 | 4.5 | 180 |
| **Total** | **~44.5** | **~48** | **~6.5** | **~435** |

**Post-workout protein lands at ~44.5 g against the 40–50 g ask — on target.** Shake composition assumed as his standard build (whey scoop + banana, water-blended); recorded per the athlete's standing 31 July decision that bars and shakes stay.

**Running total: ~52.5 P / ~69 C / ~11 F / ~585 kcal.**

**Protein still to find: ~138–163 g.** Same shape as yesterday — the noon meal has to be the anchor, and yesterday proved it works.

### Targets today

| | Target | Still to find |
|---|---|---|
| Protein | 190 to 215 g | **~182 to 207 g** |
| Carbs | 250 g cap | ~229 g of room |
| Fat | 85 to 95 g | **~80 to 90 g** |
| Calories | 2565 to 2735 | **~2415 to 2585** |
| Fibre | ~25 g | ~24 g, **plus psyllium starting today** |
| Water | 2 L+, repeat of yesterday's 2.5 L | Essential with psyllium |

---

## 4. Training: ARMS — COMPLETED

**Warmup: 5–6 min incline walk, then EZ ramping sets.**

### Session log, as performed

| Exercise | Set | Load | Reps | RIR | Note |
|---|---|---|---|---|---|
| EZ curl | W1 | 17.5 kg | 12 | — | Warmup |
| EZ curl | W2 | 20 kg | 12 | — | Warmup |
| EZ curl | 1 | 27.5 kg | 12 | 2 | +2 reps on last week at same load |
| EZ curl | 2 | **32.5 kg** | 9 | 2 | No 2.5 kg increment available — skipped 30 entirely |
| EZ curl | 3 | **32.5 kg** | 8 | 2 | Clean drop-off |
| Cable curl, single-arm | 1 | **Pin 20** | 10 | 2 | 5-pin jump absorbed at full reps |
| Cable curl, single-arm | 2 | Pin 20 | 10 | 2 | Flat reps again |
| Cable curl, single-arm | 3 | Pin 20 | **12** | **1** | Deliberate test set to true limit |
| Hammer curl, **simultaneous** | 1 | 12 kg per DB | 14 | 2 | No 12.5s in the rack |
| Hammer curl, simultaneous | 2 | **14 kg per DB** | 12 | 2 | |
| Hammer curl, simultaneous | 3 | 14 kg per DB | 12 | 2 | No drop-off |
| Rope pushdown | 1 | 80 lb | **15** | 2 | Last week's flat 12s exposed |
| Rope pushdown | 2 | **90 lb** | 11 | 2 | Clean pauses, last rep held 3+ sec |
| Rope pushdown | 3 | 90 lb | 10 | 2 | |
| Overhead cable ext, two-handed | 1 | **70 lb** | 12 | 2 | **Shoulder quiet.** Triceps burn only |
| Overhead cable ext, two-handed | 2 | 70 lb | 10 | 2 | |
| Kickback machine, two handles | 1 | 12.5 kg | 10 | 2 | **Different machine — new baseline, not comparable to last week's 40 lb** |
| Kickback machine, two handles | 2 | 12.5 kg | 8 | 2 | Very difficult; end of session |

**~16 working sets. Left shoulder and biceps insertion quiet the entire session, including the overhead position. No shrugs, no upright rows, per constraint.**

### Every single exercise progressed

| Exercise | Last week | **Today** |
|---|---|---|
| EZ curl | 27.5 kg × 10 | **32.5 kg × 9** |
| Cable curl | Pin 15 × 15 | **Pin 20 × 12** |
| Hammer curl | 10 kg × 12 alternating-unspecified | **14 kg × 12 simultaneous** |
| Rope pushdown | 80 lb × 12 flat | **90 lb × 11 with pauses** |
| Overhead extension | 60 lb × 15 | **70 lb × 12** |
| Kickback | 40 lb × 15 | New machine, 12.5 kg baseline |

**The hammer curl style note matters:** today's sets were both arms simultaneously — continuous tension, no per-arm micro-rest — which is the harder version. The 10 → 14 kg jump understates the real progression.

**The RIR calibration keeps proving itself.** The pin-20 test set (12 at 1 RIR after two flat sets of 10 at "2 RIR") shows the burn-termination habit still hides ~2–3 reps. Load decisions should keep leaning on drop-off patterns, not first-set feel.

### Next Arms session targets

| Exercise | Target | Basis |
|---|---|---|
| **EZ curl** | **32.5 kg × 9–10**, then 35 when 10 holds twice | 9 and 8 today |
| **Cable curl** | **Pin 22–23** (or 25 if the stack jumps in fives), expect 8–9 | True 13 RM at pin 20 |
| **Hammer curl** | **14 kg, aim 13–14 simultaneous**, then 16s | 12 × 3 flat today |
| **Rope pushdown** | **90 lb, aim 12–13 with pauses** | 11 and 10 today |
| **Overhead ext** | **70 lb, aim 13–15**, then 80 | 12 and 10, shoulder quiet |
| **Kickback machine** | **12.5 kg, aim 10–12 both sets** | New baseline set today |

**Tomorrow, Friday: LEGS.** Leg press 190 kg × 8 at 2 RIR.

---

## 5. Open Items Today

- [ ] **START PSYLLIUM TODAY.** 1 tsp in a full glass of water with a meal. The bar test has read out; nothing left to confound
- [ ] **Repeat yesterday's ~2.5 L of water.** Psyllium demands it
- [ ] **Do not strain.** If it will not come, wait for the post-meal urge
- [ ] **Eat a real midday meal.** Arms day on a 150 kcal opening; protein-at-noon is the proven structure
- [ ] **Report the headache status** — better, same, or worse than yesterday
- [ ] **Report sleep with a wake time**
- [ ] **Blood pressure reading — still outstanding**, and still the one worth doing
- [ ] **Arms session: report loads live.** No shrugs, no upright rows
- [ ] If no meaningful movement by Sunday despite psyllium and water, **raise magnesium citrate with Dr Chalhoub**

---

*Log maintained daily. Interpretation is coaching context only. All diagnosis, dosing and treatment decisions rest with Dr Clarissa Chalhoub.*

# MEC Daily Training & Nutrition Log

**Date:** Wednesday, 5 August 2026
**Athlete:** Mohamad Eid Chmaitilli (MEC)
**Prepared for:** Cal (personal trainer) / coaching team
**Status:** CLOSED AND CONFIRMED. All four meals verified by the athlete.

---

## 1. Athlete Snapshot

| Field | Value |
|---|---|
| Weight | **Not reported today** |
| Last measured | 88.9 kg, empty stomach, 2 August |
| Session | **NONE.** Early meeting, then delivering training at Bank Audi |
| Sleep | **Bedtime 23:00 to 23:30.** First sleep data in five nights. Woke ~1 h later than usual, by choice, because of the headache |
| Estimated duration | **~7 to 7.5 h** against a normal ~06:35 weekday wake |
| Water | **~2.5 L.** 1.0 L by evening, plus ~1.5 L after. **Above his 2 L+ normal — best logged figure on record** |
| Vitamin D | **Dose 3 TAKEN.** Cleared |
| Bowel movements | **1**, morning only. Half formed, loose at the end. Good relief, no straining |
| Enhancement status | **On cycle — boldenone and testosterone** |
| Active complaint | **Bilateral headache and trap-to-neck pain**, all day |
| Phase | Diet break. **Cut begins Saturday 8 August at ~2200 kcal** |
| Environment | Lebanese August heat. Relevant to fluid loss and appetite |

---

## 2. Active Constraints

**Training**

- [x] **NO OVERHEAD PRESSING.** Standing, permanent. Rotator cuff history, left side
- [x] **Left shoulder watch:** three consolidated findings — cuff history, ~12% strength deficit (7 left vs 8 right at 38 kg single-arm rows), anterior biceps-insertion irritation
- [x] **Dumbbell entry protocol:** the provocation is loaded external rotation getting the bells into position, not the press itself. Enter with dumbbells facing each other, or use the legs to assist
- [x] **NEW this week: no shrugs, no upright rows** while the trap/neck pain is active

**Nutrition**

- [x] **No whey**
- [x] **No labneh**
- [x] **No red meat**
- [x] **No fish**
- [x] ~~No oats~~ — **CLEARED by the athlete on 4 August.** Brookies and protein cookies contain oat flour and are now permitted
- [x] **Carbs: 250 g hard cap**
- [x] **Protein: 190 to 215 g**
- [x] **Caffeine under 400 mg.** Two breaches this week, the medium Pike driving both
- [x] **Polyol-containing bars removed since 4 August.** Test in progress, see §3
- [x] **Saturated/dairy fat limited.** LDL 161.4. Fat should come from olive oil and walnuts

**Medical context**

- [x] **On cycle: boldenone and testosterone.** Affects the 28 July panel's testosterone, lipid and creatinine sections
- [x] **Dr Chalhoub has not been told.** Top of the standing open list

---

## 3. Issue Status

### 3a. Fatigue — QUIET

**No fatigue reported today.** The athlete stated explicitly on 3 August: *"No I am not tired."* The extra hour of sleep this morning was taken for the headache, not for tiredness.

**Sleep is the input that has actually moved.** A 23:00–23:30 bedtime against a ~06:35 wake gives ~7 to 7.5 h, versus the ~6 h 27 m 2026 average on record. **This removes the one caveat that could have argued for delaying Saturday's cut.**

**Still missing: wake times.** Bedtime is half the number.

### 3b. GI — IMPROVING, NOT RESOLVED

**Day 2 of the bar removal.**

| Date | Passages | Consistency |
|---|---|---|
| 31 July | **3** | — |
| 3 August | 2 | **Swooshy throughout**, both |
| 4 August | 2 | Swooshy AM, **two formed pieces** PM |
| **5 August** | **1, morning only** | **Half formed, loose at the end** |

**Both variables are moving the same way: frequency falling 3 → 2 → 1 while consistency firms.** That is the direction of travel expected from removing an osmotic irritant.

**One morning passage with good relief and no straining is the target state, not a shortfall.** Normal frequency runs from three times a day to three times a week. Three in one day on 31 July was the outlier — that reading is what killed the slow-transit hypothesis, because it fails the frequency criterion for functional constipation outright.

**Formed-first, loose-last is itself informative.** The leading portion has spent longer in the colon with more water absorbed; the tail is more recent material moving faster than it should. As transit slows toward normal, the loose fraction shrinks from the back. **That is what partial resolution looks like.**

**One passage on a normally-fed day is a stronger positive read than one passage on a starved day.** An earlier version of this entry attributed the single passage partly to there being little in the system to move. **That reasoning is withdrawn** — the pizza landed at midday, so there was a substantial meal in the system by early afternoon and the gastrocolic reflex had something to fire on. **The single, well-formed, strain-free passage therefore reflects transit itself, not an empty gut.**

#### Tomorrow's day-three read is a GOOD test after all

**This file twice said tomorrow's read would be noisy, on the basis that the day ran to 18:00 on ~220 kcal and then closed on one large low-fibre meal. That was wrong on the facts and the caveat is withdrawn.**

**Today was in fact well distributed:** ~220 kcal at breakfast, ~1005 at noon, ~683 mid-evening, ~440 to close — with **~21 g of fibre**, the highest since the bars were removed. That is a normal, representative day.

**So tomorrow morning is a clean day-three read, and it carries its full weight.** Continued firming favours polyols as the driver. A plateau at half-formed favours the fibre ramp still needing its full one to two weeks.

**No action changes: bars stay out, psyllium starts Friday 7 August.**

### 3c. Headache and trap/neck pain — NEW TODAY

**Reported: constant bilateral headache, plus pain in the traps ascending into the neck. Slept an extra hour because of it.**

**Chin to chest is pain-free. That rules out meningism** — resistance to forward flexion is the sign separating a mechanical neck problem from meningeal irritation. This is musculoskeletal and environmental, not a red-flag headache.

| Rank | Cause | Evidence |
|---|---|---|
| **1** | **Cold draft — two hours of car AC directly on the head** | Sustained cold air on the scalp, neck and upper traps produces local guarding in the cervical extensors and upper trapezius. Referred pain from those muscles is **bilateral, band-like, ascending from traps into the occiput** — precisely the described distribution. Next-day onset is typical |
| **2** | **90 minutes of static night work** | Head-forward posture loads the same muscles the draft already tightened. The two stack |
| **3** | **Caffeine** | ~450 mg Monday, sharply lower since. Withdrawal headache is bilateral, 12–24 h after a drop. Contributory, not primary |
| **4** | **Blood pressure** | **The one worth actually measuring.** Both compounds raise BP; bilateral headache is its classic presentation. Last reading 115/68, timing relative to the cycle unknown |
| **5** | Elevated estradiol | Both compounds aromatise. Consistent with the facial puffiness, but does not explain trap pain |

**The AC exposure leads because it is specific, time-locked, and the only item that explains the trap-to-neck distribution.** Training load was the earlier front-runner and now drops — Monday's back session and Tuesday's shoulders were heavy, but he has trained heavy for weeks without this.

**Management:** heat not ice on the traps and base of skull, 10–15 min. Chin tucks and slow rotations through the day, gentle range. **Redirect the car vents rather than turning the AC off** — the Lebanese heat is why it was blasting in the first place. Hold caffeine steady rather than dropping further while the headache is live.

**Escalate to Dr Chalhoub if** it becomes the worst he has had, or arrives with visual disturbance, fever, weakness or numbness, or fails to improve over 48 hours.

---

## 4. Training Log

### Today: NO SESSION

**Early meeting, then delivering training at Bank Audi. Legs was scheduled and did not happen.**

**A missed session is not a problem here.** Four resistance sessions in the week with every exercise progressing, on an 18-hour workday. Accessories have been deliberately trimmed since 25 July while fatigue was managed. **One displaced leg day is well inside tolerance.**

### The rotation shifts, and it costs nothing

| Day | Was | **Now** |
|---|---|---|
| Wed 5 Aug | Legs | **No session** |
| Thu 6 Aug | Arms | **Arms — unchanged** |
| Fri 7 Aug | Cardio + core, optional | **LEGS** |
| Sat 8 Aug | Chest | Chest. **Cut begins** |

**Cardio + core was already optional during the diet break, so nothing is lost — the week keeps all four resistance sessions.**

### Next session: Thursday 6 August — ARMS

| Exercise | Target | Note |
|---|---|---|
| **EZ bar curl** | **30 kg × 8–10, 2 RIR** | **Confirmed anchor.** Set yesterday when the shoulder read quiet |
| Remaining arm work | **Carry loads forward from 31 July and log live** | Not pre-set here — the 31 July numbers are the reference, and I will not invent targets I cannot anchor |
| **Shrugs, upright rows** | **OMITTED** | Trap/neck pain active. Reinstate once it settles |

**Watch the left biceps insertion.** The anterior irritation reported Monday sits exactly where curls load. **If it speaks up, drop to a neutral or hammer grip before dropping load** — the long head and the biceps pulley are the suspects, and grip position changes their line of pull.

### Friday 7 August — LEGS

| Exercise | Target |
|---|---|
| **Leg press** | **190 kg × 8, 2 RIR** |

**190 is deliberate.** 200 kg went to 0 RIR against plan on 30 July, so that number is not usable as a working anchor.

---

## 5. Nutrition Log

### Meal 1 — morning

| Item | P | C | F | Fibre | kcal |
|---|---|---|---|---|---|
| Brookie | 9 | 12 | 3 | ~1 g | 120 |
| Protein cookie | 10 | 12 | 3 | ~1.5 g | 100 |
| **Total** | **19** | **24** | **6** | **~2.5 g** | **~220** |

**Polyols: zero. Second consecutive bar-free day.**

#### Third consecutive light opening

| Date | Meal 1 |
|---|---|
| 2 August | **~1048 kcal**, ~10.5 g fibre |
| 3 August | ~219 kcal |
| 4 August | ~130 kcal |
| **5 August** | **~220 kcal** |

**2 August remains the only properly fed start in four days.** The heat is a real contributor — it suppresses appetite, and a 220 kcal breakfast in Lebanese August is not surprising.

**But the light opening did not cost anything today, because the midday meal was large.** Protein landed at noon rather than at breakfast. That is a viable structure and it worked.

### Meal 2 — NOON: high-protein pizza with 300 g chicken

| Item | P | C | F | Fibre | kcal |
|---|---|---|---|---|---|
| Grilled chicken, 300 g | ~93 | 0 | ~11 | 0 | ~495 |
| Pizza base, cheese, sauce | ~31 | ~55 | ~19 | ~3 g | ~510 |
| **Total** | **~124** | **~55** | **~30** | **~3 g** | **~1005** |

**ESTIMATED, and recorded as such at the athlete's instruction.** The vendor does not list macros. **The chicken figure is reliable; the base and cheese are inferred and could swing protein by ±20 g.**

**This meal was eaten at NOON, not in the evening.** An earlier version of this entry placed it as dinner and built several conclusions on that — that the day ran to 18:00 on ~220 kcal, that this was "the meal that rescued it", and that the intake pattern made tomorrow's GI read unreliable. **All three are withdrawn.** Protein landed at midday and the day was well distributed from there.

### Meal 3 — evening: Pasta Beast Salad, no parmesan

**Vendor listing: 748 kcal, 55 g protein.** Romaine, rocca, pasta, grilled corn, mushrooms, red peppers, boiled eggs, chicken breast, avocado-mustard dressing. **Parmesan removed at the athlete's request.**

| Item | P | C | F | Fibre | kcal |
|---|---|---|---|---|---|
| Pasta Beast Salad as listed | 55 | ~55 | ~34 | ~7 g | 748 |
| Less parmesan (~15 g) | −5 | 0 | −5 | 0 | −65 |
| **As eaten** | **~50** | **~55** | **~29** | **~7 g** | **~683** |

**Protein and calories are vendor-listed and reliable. Carbs and fat are back-solved** — the app showed only those two figures, the rest sat behind the collapsed "View Nutrition Facts" panel. 55 P is 220 kcal; the remaining 528 kcal split across the pasta and corn on one side and the dressing and eggs on the other gives ~55 C and ~34 F, which reconciles to 746 against a listed 748.

**Dropping the parmesan was the right call** — LDL 161.4 and two consecutive days already weighted toward dairy fat.

**This is a good find and it goes into the database.** ~50 g protein, ~7 g fibre and real vegetables for $13. **On fibre it beats the Protein Sandwich outright**, which is the gap this week keeps running into.

**One ask: open the Nutrition Facts dropdown next time.** One tap removes the estimate on carbs and fat entirely.

**The Protein Sandwich was recommended earlier and was not needed.** The salad covered the same protein gap with seven times the fibre.

### Meal 4 — walnuts and figs

| Item | P | C | F | Fibre | kcal |
|---|---|---|---|---|---|
| Walnuts, 45 g | ~7 | 4 | 30 | ~3 g | ~295 |
| Dried figs, 60 g | ~2 | 38 | 0.5 | ~5.6 g | ~145 |
| **Total** | **~9** | **~42** | **~30.5** | **~8.6 g** | **~440** |

**Quantities assumed at the recommended 45 g and 60 g.** The athlete reported "the walnut & the figs" without weights. **Vitamin D was taken — pairing it with 30 g of walnut fat is the best absorption window the day offered.**

### Day Close Against Targets — CONFIRMED

**CONFIRMED. All four meals eaten.**

| Meal | Time | P | C | F | Fibre | kcal |
|---|---|---|---|---|---|---|
| 1. Brookie + protein cookie | Morning | 19 | 24 | 6 | ~2.5 g | ~220 |
| 2. Pizza, 300 g chicken | **Noon** | ~124 | ~55 | ~30 | ~3 g | ~1005 |
| 3. Pasta Beast Salad, no parmesan | Evening | ~50 | ~55 | ~29 | ~7 g | ~683 |
| 4. Walnuts 45 g + figs 60 g | Late | ~9 | ~42 | ~30.5 | ~8.6 g | ~440 |
| **DAY TOTAL** | | **~202** | **~176** | **~95.5** | **~21 g** | **~2348** |

| | Target | Close | Verdict |
|---|---|---|---|
| Protein | **190 to 215 g** | **~202 g** | ✅ **In band** |
| Carbs | **250 g cap** | **~176 g** | ✅ **74 g of room** |
| Fat | **85 to 95 g** | **~95.5 g** | ✅ **At the ceiling** |
| Calories | 2565 to 2735 | ~2348 kcal | ~220 under |
| Fibre | ~25 g | **~21 g** | 4 g short. **Best since the bars came out** |
| Water | 2 L+ | **~2.5 L** | ✅ **Best figure on record** |
| Vitamin D | Dose 3 due | **Taken** | ✅ |

**Three of the four macro targets land inside their bands on a day with no training and no plan.**

**The ~220 kcal shortfall is immaterial** and does not need chasing. On a rest day, three days before a cut, with protein covered and fat at ceiling, the difference is noise.

**Fibre at ~21 g is the number worth noting.** It has not been near this since the polyol bars were removed, and it got there from real food — the salad's vegetables and pasta, plus the figs and walnuts — rather than from a fortified product. **That is the pattern to repeat, and it makes Friday's psyllium a top-up rather than a rescue.**

#### What made this day work

**Protein landed at noon, not at night.** The 220 kcal breakfast did not matter because the midday meal was large. **This is the second structure in the file that works** — 2 August did it by front-loading breakfast; today did it by front-loading lunch. Both beat the pattern of arriving at 21:00 with 150 g still to find.

**The salad did the job the Protein Sandwich was recommended for, and did it better.** Same protein gap closed, seven times the fibre, and it displaced a second dairy-fat serving. **It should become the default delivery order.**

---

## 6. Verified Food Database

**Label-confirmed or athlete-confirmed. Estimates are marked.**

| Item | Serving | P | C | F | Fibre | kcal | Source |
|---|---|---|---|---|---|---|---|
| **Protein Sandwich** | 175 g | **67.7** | **21** | ~23 | ~1 g | **559.9** | Label, athlete photo |
| **Pasta Beast Salad, no parmesan** | 1 bowl | **~50** | ~55 | ~29 | ~7 g | **~683** | **Vendor P and kcal listed; C and F back-solved.** $13. Best fibre-per-dollar protein meal in the database |
| **Brookie** | 1 | 9 | 12 | 3 | ~1 g | 120 | Label. **Contains oat flour** |
| **Protein cookie** | 1 | 10 | 12 | 3 | ~1.5 g | ~100 | Athlete-confirmed. **Contains oats** |
| **Chelal cheese, low sodium** | 100 g | 22 | 5 | 22 | 0 | ~300 | Label. Sodium 1.1 g |
| **Grilled chicken breast** | 100 g | ~31 | 0 | ~3.6 | 0 | ~165 | Standard |
| **Walnuts** | 30 g | 4.5 | 4 | 20 | ~2 g | ~196 | Standard |
| **Dried figs** | 60 g | ~2 | ~38 | 0.5 | ~5.6 g | ~145 | Standard |
| **Banana** | ~120 g | 1.3 | 27 | 0.4 | ~3 g | ~105 | Standard |
| **Daoukiyeh cup** | 1 cup | ~25 | ~8 | ~2 | 0 | ~150 | Prior entry. **Unavailable today** |
| **Pepsi Zero** | 1 can | 0 | 0 | 0 | 0 | 0 | Label |
| **Tannour bread** | 1 | ~6 | ~30 | ~1 | ~1.5 g | ~155 | Standard |
| **~~Protein Crisp bar~~** | — | — | — | — | — | — | **REMOVED 4 August. Polyols** |

**Supplements**

| Item | Detail | Status |
|---|---|---|
| **MagneFix** | **Magnesium bisglycinate 300 mg + vitamin B6 25 mg** | Nightly, for sleep. **Not an osmotic laxative** — chelated, absorbed in the small intestine |
| **Vitamin D** | Dose 3 due **today, 5 August** | **STILL OUTSTANDING.** Take with the walnuts — fat-soluble, and 30 g of fat is the best absorption window today |
| **Psyllium husk** | Not started | **Starts Friday 7 August**, deliberately deferred so it does not confound the bar test |

**Open question carried forward:** is the MagneFix 300 mg the compound or the elemental magnesium? Roughly a tenfold difference in effective dose.

---

## 7. Weekly Pro Split

| Day | Session | Status |
|---|---|---|
| Sat 1 Aug | **Chest** | ✅ Completed. DB press to 40 kg × 7 |
| Sun 2 Aug | Rest | ✅ |
| Mon 3 Aug | **Back** | ✅ Completed. Pulldown 100 kg × 8 |
| Tue 4 Aug | **Shoulders** | ✅ Completed. No overhead pressing |
| **Wed 5 Aug** | ~~Legs~~ → **Rest** | ✅ **Displaced.** Bank Audi delivery day |
| Thu 6 Aug | **Arms** | Scheduled. EZ curl 30 kg anchor |
| Fri 7 Aug | **Legs** | **Moved here.** Leg press 190 kg × 8 |
| Sat 8 Aug | **Chest** | **CUT BEGINS. ~2200 kcal, protein held 190–215** |

**Four resistance sessions preserved this week despite the displacement.** No overhead pressing appears anywhere in the split, by standing constraint.

---

## 8. Open Items for the Trainer

- [ ] **DISCLOSE THE CYCLE TO DR CHALHOUB.** Top of the standing list. The 28 July panel was interpreted without it
- [ ] **Add to the next draw:** estradiol, LH, FSH, free testosterone, SHBG, prolactin. **Repeat lipids off-cycle**
- [ ] **TAKE A BLOOD PRESSURE READING.** No on-cycle reading exists anywhere in the file, and bilateral headache is its classic presentation
- [x] **VITAMIN D DOSE 3 — TAKEN.** Cleared
- [x] **Day close confirmed.** All four meals eaten. **~202 P / ~176 C / ~95.5 F / ~2348 kcal, ~21 g fibre**
- [ ] **Open the "View Nutrition Facts" dropdown** on the salad next time — one tap removes the carb and fat estimate
- [ ] **Make the Pasta Beast Salad the default delivery order.** ~50 g protein and ~7 g fibre for $13, and it displaces a dairy-fat serving
- [x] **Water — ~2.5 L. CLEARED**, and the best figure on record
- [ ] **Heat on the traps, chin tucks through the day.** Redirect the car vents rather than switching the AC off
- [ ] **No shrugs, no upright rows Thursday** while the trap/neck pain is active
- [ ] **Report tomorrow's GI read — but weight it low.** Today's intake was not representative
- [ ] **Log wake times, not just bedtimes.** Bedtime is half the number
- [ ] **Start psyllium Friday 7 August**, once the bar test has read out
- [ ] **Legs on Friday, Arms Thursday.** Confirm both fit around the work schedule
- [ ] **Caffeine under 400 mg.** Two breaches this week
- [ ] **Confirm whether red meat and labneh are permanent exclusions or one-offs.** Carried unanswered for over a week
- [ ] **Check whether MagneFix 300 mg is compound or elemental magnesium**

---

*Log maintained daily. Interpretation is coaching context only. All diagnosis, dosing and treatment decisions rest with Dr Clarissa Chalhoub.*
